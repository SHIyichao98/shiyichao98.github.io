...
const splitGallery = (meta) => {
  ...
  // that declares a full-width wall and no grid, or a hub of other pages, has
  // made its choice, so the cover must not reappear as a lone grid tile.
  const fallback = meta.gallery_full || meta.hub ? "" : meta.cover;
...
const showIndex = (updateHash = true, wall = null, category = null) => {
  ...
  // Two tracks. The homepage and the Teaching heading show only the chosen
  // nine; a category view shows the extra tiles as well.
  document.querySelectorAll(".gallery .tile[data-category]").forEach((tile) => {
    const extra = tile.hasAttribute("data-extra");
    tile.hidden = category ? tile.dataset.category !== category : extra;
  });
